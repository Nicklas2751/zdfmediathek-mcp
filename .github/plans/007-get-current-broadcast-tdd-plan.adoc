...
Implementierung eines MCP-Tools `get_current_broadcast`, das die aktuell laufende Sendung eines bestimmten ZDF-Senders zurückgibt.
...
* `limit` (Int, optional, default: 10): Maximale Anzahl der Broadcasts, die vom API abgerufen werden (erhöht die Wahrscheinlichkeit, die aktuell laufende Sendung zu finden)
...
* Service-Klasse mit `@Service` und `@Tool`-Annotation
...
* **Tatsächlich**: ~2.5 Stunden (inkl.
Diskussion und Fehlerbehebung)
* **Geplant**: ~2 Stunden
