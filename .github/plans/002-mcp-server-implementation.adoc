...
- Tools (functions that AI can call) - Resources (data that AI can read) - Prompts (templates for AI interactions) - Transport (stdio vs HTTP)
...
**Task:**
1. Research the latest version of Kotlin MCP SDK - Check JetBrains repositories or GitHub - Find the Maven coordinates
...
- Core client - Engine (CIO recommended for JVM) - Content negotiation (for JSON) - Logging
...
**Task:**
1. Update `App.kt` to:
- Initialize MCP server - Configure ZDF API client - Start server with stdio transport - Handle graceful shutdown
...
- Console appender - JSON format (for production) - Appropriate log levels - Context information
...
2. Add:
- Build job - Test job - Coverage job - Docker build job 3. Test workflow
...
**Phase 5:**
- [ ] Integration tests passing - [ ] Code coverage ≥ 80% - [ ] No Detekt issues
